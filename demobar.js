...
export default class Demobar extends React.Component {
  ...
  closeModal = () => {
    this.setState({ modalOpen: false, formName: '' });
  };
  ...
  saveFormData = async () => {
    const { data, formName } = this.state;
    // const updatedData = {
    //   task_data: [...data] ,
    //   name: { formName }
    // };
    // await this.setState({ data: updatedData });
    // console.log("Updated Data to saveName:", this.state.data.name.formName);
    // console.log("Updated Data to save.task_data:", this.state.data.task_data);
    // { data: this.state.data.task_data}
    await store.dispatch('post',{data,formName});
  };
  ...
  render() {
    ...
    return (
      <div className="clearfix" style={{ margin: '10px', width: '70%' }}>
        ...
        <button className="btn btn-default float-right" style={{ marginRight: '10px' }} onClick={() => this.openModal()}>Save Form</button>
        ...
        <Modal
              ...
              onFormNameChange={(name) => this.setState({ formName: name })}
            />
